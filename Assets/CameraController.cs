using UnityEngine;

public class CameraController : MonoBehaviour
{
    public Transform target;
    public float defaultDistance = 5.0f;
    public Vector3 defaultPosition;
    public float zoomSpeed = 0.1f;
    private float wantedDistance = 5.0f;
    private float currentDistance = 5.0f;
    public float panSpeed = 0.1f;
    public float returnPanSpeed = 0.1f;
    private Vector3 wantedPosition = new Vector3();
    private Vector3 currentPosition = new Vector3();
    public float xSpeed = 120.0f;
    public float ySpeed = 120.0f;
    public float zSpeed = 20.0f;
    public float yMinLimit = -90f;
    public float yMaxLimit = 90f;
    public float distanceMin = .5f;
    public float distanceMax = 15f;
    public Vector2 firstMousePosition;
    public int unlockThreshold = 5;
    public bool spinUnlocked = false;
    public Vessel vessel;

    private float x = 0.0f;
    private float y = 0.0f;

    private void Start()
    {
        Initialize();
    }

    public void Initialize()
    {
        Vector3 angles = transform.eulerAngles;
        x = angles.y;
        y = angles.x;

        currentDistance = defaultDistance;
        wantedDistance = defaultDistance;
    }

    private void LateUpdate()
    {
        if (target)
        {
            if (Input.GetKeyDown(KeyCode.C))
            {
                firstMousePosition = Input.mousePosition;
            }

            if (Input.GetKey(KeyCode.C))
            {
                if (!spinUnlocked)
                {
                    if (Vector2.Distance(Input.mousePosition, firstMousePosition) >= unlockThreshold)
                    {
                        spinUnlocked = true;
                    }
                }
            }

            if (Input.GetKeyUp(KeyCode.C))
            {
                spinUnlocked = false;
            }

            if (spinUnlocked)
            {
                x += Input.GetAxis("Mouse X") * xSpeed * 0.02f;
                y -= Input.GetAxis("Mouse Y") * ySpeed * 0.02f;

                y = ClampAngle(y, yMinLimit, yMaxLimit);

                Quaternion rotation = Quaternion.Euler(y, x, 0);

                wantedDistance = Mathf.Clamp(wantedDistance - Input.GetAxis("Mouse ScrollWheel") * zSpeed, distanceMin, distanceMax);

                currentDistance = Mathf.Lerp(currentDistance, wantedDistance, zoomSpeed);

                Vector3 negDistance = new Vector3(0.0f, 0.0f, -currentDistance);

                wantedPosition = target.position;
                currentPosition = Vector3.Lerp(currentPosition, wantedPosition, panSpeed);

                Vector3 position = rotation * negDistance + currentPosition;

                transform.rotation = rotation;
                transform.position = position;

                transform.rotation = Quaternion.Euler(transform.rotation.eulerAngles);
            }
            else
            {
                Vector3 offset = Vector3.zero;
                if (vessel)
                {
                    offset = vessel.moveInputs * 0.01f;
                    if (vessel.mainburn)
                    {
                        offset += Vector3.forward * 0.1f;
                    }
                }

                //if (vessel.retroburn)
                //{
                //    //transform.rotation = Quaternion.Lerp(transform.rotation, Quaternion.Euler(new Vector3(0, 180, 0)), returnPanSpeed);
                //    transform.LookAt(target);
                //    transform.localPosition = Vector3.Lerp(transform.localPosition - offset, new Vector3(-defaultPosition.x, defaultPosition.y, -defaultPosition.z), returnPanSpeed);
                //}
                //else
                //{
                    transform.rotation = Quaternion.Lerp(transform.rotation, Quaternion.Euler(target.rotation.eulerAngles), returnPanSpeed);
                    transform.localPosition = Vector3.Lerp(transform.localPosition - offset, defaultPosition, returnPanSpeed);
                //}
            }
        }
    }

    public float ClampAngle(float angle, float min, float max)
    {
        if (angle < -360F)
        {
            angle += 360F;
        }
        if (angle > 360F)
        {
            angle -= 360F;
        }
        return Mathf.Clamp(angle, min, max);
    }
}
